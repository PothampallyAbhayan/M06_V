...
/*
 * Possible values for DB storage
 */
#define DB_STORAGE_SQLITE                       0x00
#define DB_STORAGE_INMEMORY                     0x01

typedef struct _nos_db_handle_
{
    unsigned char    db_storage;
    void             *modbus_mem_handle;
    char             *mapped_address;
    sqlite3          *db_handle;
    int              db_type;
    int              shmid;
...
typedef struct _icos_in_mem_handle_
{
    struct _icos_in_mem_handle_  *next;
    int      mem_key_id;
    char     *mapped_address;
...
NOS_DB_HANDLE  *nos_db_init(int type);
NOS_DB_HANDLE  *nos_db_get_db_handle();
int nos_db_close(void);
...
int nos_db_perform_cmd(NOS_DB_HANDLE  *handle, char *cmd);
