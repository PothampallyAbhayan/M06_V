...
int get_adc_avg(int adc_flag, int chn_addr, int chn_no,int *avg_value, int *off_count, int *str_count, int *fan_rec_count);


#define NO_OF_SBOARDS	    4
                    // Function to find the bit position
...
#define DB_FIELD  1
#define SHM_FIELD 2
#define FIELD_TOTALENTRY 2500  // Changed from 1400 to 2000    
...
unsigned char wMboard_Fw_Ver[16] = {'M','-','0','5','F','C',' ',' ',' ',' ',' ',' ',' ',' ',' ',' '};
/**************BACnet ******************************************************************/
pthread_t bacnet_thread;
pthread_t update_thread;
/*****************Stack support variable and Functions************************************************/
...
/***End slave id reading*****/
    ...
    { 
      ...
      do
      {
        read_fd =open("/etc/BACid",O_RDONLY|S_IRUSR);
    ...
    //log_init(); //Commented on 27 sept 21
   ...
   // create_db_field(p_db,DB_FIELD);//For DB//DK removed
...
/************************************ END ***********************************************************************************/ 
	  ...
	  } else
	  {
	    if ( FD_ISSET(thread_ctx.sock_adc,&temp_RD_fd) ) 
	    {
		  ...
		  Data.word.System_Status.Fan_status = (unsigned short) ((status_result) & 0x00ff);  //12345 earlier was 0x009f-- modified to have 8 fans instead of 6
                  //printf("\nFan status (in hex) : %x\n",Data.word.System_Status.Fan_status);    //12345 
			...
			if (no_xfmr)
			{
				...
				{ 
					...
					// else
					// { 
					//       fan_rec_cntr++;                         //3216212 Fan recovery counter
					//   if(fan_rec_cntr > fan_cntr_Parameters[3])
					//{
					if(event_flag[3])
					{
						//printf("\nFan alarm recover\n");
				...
				{
					// if(fan_rec_cntr > 0)                             ///3216212 Fan recovery counter set to 0
					// {
					// fan_rec_cntr = 0;
					// //printf("\nprinting1 %d:\n",Fan_alarm_cntr);
					// }

					// if (Fan_alarm_cntr < fan_cntr_Parameters[4])
					// {
					// Fan_alarm_cntr++;
					// //printf("\nprinting2 %d:\n",Fan_alarm_cntr);
					// }
...
void * shm_filling(void* arg)
{
    ...
    while(1)
    {
      ...
      for (dummy_cntr =0;dummy_cntr<(FIELD_TOTALENTRY);dummy_cntr++)                
      {
	      ...
	      //printf("\nReg_Value : %x\tArray_Index : %x\n",db_reg_value[dummy_cntr],Arr_ifindex[dummy_cntr]);  //changed on 03/10/21 for checking
...
void transmit_pkt(void )
{
    ...
    //printf("Panel Overcurrent(85-101) = %d \n", Data.array[PANEL21_UNDERCURR_STATUS_FLAG]);
//    printf("Panel Overcurrent(101 - 1)  = %d \n", Data.array[PANEL22_UNDERCURR_STATUS_FLAG]);
    

	//printf("\n Data.word.Primary.KWH_Hi = %x", Data.word.Primary.KWH_Hi);  //12345
	//printf("\n Data.word.Primary.KWH_Lo = %x", Data.word.Primary.KWH_Lo);  //12345
	
	/*nbytes_uart = write(uart_fd,write_buffer,sizeof(write_buffer));
...
void KWH_Calc(unsigned int reg)
{
	...
	switch (reg)
	{
	  case PRIMARY:
		...
		if((Data.word.Primary.KW & 0x0000FFFF) > 0xEA60)
		...
		*(ptKWH_Error) = dwMathBuff % 3600;
		*(map + KWH1_ERROR) = *(ptKWH_Error);
		dwMathBuff = (dwMathBuff / 3600);
		...
		*(map + 11) = (0x080A0000 | dwCalcBuff[0].word[0]);
		
		Data.word.Primary.KWH_Hi = (0x080B0000 | dwCalcBuff[0].word[1]);
		*(map + 10) = (0x080B0000 | dwCalcBuff[0].word[1]);
		
		Reg[11].reg_d.reg_value = (dwCalcBuff[0].word[0]);
		Reg[10].reg_d.reg_value = (dwCalcBuff[0].word[1]);
	  ...
	  case SEC:
		...
		if((Data.word.Secondary.KW_Phase_A& 0x0000FFFF) > 0x4E20)
		...
		*(map + PRIMARY_OFFSET + 11) = (0x090A0000 | dwCalcBuff[1].word[0]);
		
		Data.word.Secondary.KWH_Phase_A_Hi = (0x090B0000 | dwCalcBuff[1].word[1]);
		*(map + PRIMARY_OFFSET + 10) = (0x090B0000 | dwCalcBuff[1].word[1]);
		
		Reg[PRIMARY_OFFSET + 11].reg_d.reg_value = (dwCalcBuff[1].word[0]);
		Reg[PRIMARY_OFFSET + 10].reg_d.reg_value = (dwCalcBuff[1].word[1]);
		

		// PhaseB
		ptKWH_Error = &KWH_Error.dwSecondary_PhaseB;
		
		if((Data.word.Secondary.KW_Phase_B & 0x0000FFFF) > 0x4E20)
		...
		*(map + PRIMARY_OFFSET + 13) = (0x090C0000 | dwCalcBuff[2].word[0]);
		
		Data.word.Secondary.KWH_Phase_B_Hi = (0x090D0000 | dwCalcBuff[2].word[1]);
		*(map + PRIMARY_OFFSET + 12) = (0x090D0000 | dwCalcBuff[2].word[1]);
		
		Reg[PRIMARY_OFFSET + 13].reg_d.reg_value = (dwCalcBuff[2].word[0]);
		Reg[PRIMARY_OFFSET + 12].reg_d.reg_value = (dwCalcBuff[2].word[1]);
		 

		//PhaseC
		ptKWH_Error = &KWH_Error.dwSecondary_PhaseC;
		
		if((Data.word.Secondary.KW_Phase_C & 0x0000FFFF) > 0x4E20)
		...
		*(map + PRIMARY_OFFSET + 15) = (0x090E0000 | dwCalcBuff[3].word[0]);
		
		Data.word.Secondary.KWH_Phase_C_Hi = (0x090F0000 | dwCalcBuff[3].word[1]);
		*(map + PRIMARY_OFFSET + 14) = (0x090F0000 | dwCalcBuff[3].word[1]);
		
		Reg[PRIMARY_OFFSET + 15].reg_d.reg_value = (dwCalcBuff[3].word[0]);
		Reg[PRIMARY_OFFSET + 14].reg_d.reg_value = (dwCalcBuff[3].word[1]);
	  ...
	  case PANEL1:
		...
		for (i=0; i<84; i++)
		{
			if((Data.array[RMS_3_OFFSET + i] & 0x0000FFFF) > 0x3AC0)
			...
			*(ptCTKwh_Error + i) = dwMathBuff % 3600;
			...
			*(map + PANEL24_OVERCURR_FLAG + (i*2)) = (Data.array[PANEL24_OVERCURR_STATUS_FLAG + (i*2)] & 0x0000FFFF);
			Reg[PANEL24_OVERCURR_FLAG + (i*2)].reg_d.reg_value = (Data.array[PANEL24_OVERCURR_STATUS_FLAG + (i*2)] & 0x0000FFFF);
			
			Data.array[PANEL24_OVERCURR_STATUS_FLAG + (i*2) + 1] = ((((0x2D00|(i*2 + 1))|(0<<14))<<16) | dwCalcBuff[4].word[1]);
			*(map + PANEL24_OVERCURR_FLAG + (i*2) + 1) = (Data.array[PANEL24_OVERCURR_STATUS_FLAG + (i*2) + 1] & 0x0000FFFF);
			Reg[PANEL24_OVERCURR_FLAG + (i*2) + 1].reg_d.reg_value = (Data.array[PANEL24_OVERCURR_STATUS_FLAG + (i*2) + 1] & 0x0000FFFF);
	  ...
	  case PANEL2:
		...
		for (i=0; i<84; i++)
		{
			if((Data.array[KW_0_OFFSET + i] & 0x0000FFFF) > 0x3AC0)
			...
			*(map + KWH0_OFFSET + (i*2)) = (Data.array[KWH_0_OFFSET + (i*2)] & 0x0000FFFF);
			Reg[KWH0_OFFSET + (i*2)].reg_d.reg_value = (Data.array[KWH_0_OFFSET + (i*2)] & 0x0000FFFF);
			
			Data.array[KWH_0_OFFSET + (i*2) + 1] = ((((0x2D00|(i*2 + 1))|(1<<14))<<16) | dwCalcBuff[5].word[1]);
			*(map + KWH0_OFFSET + (i*2) + 1) = (Data.array[KWH_0_OFFSET + (i*2) + 1] & 0x0000FFFF);
			Reg[KWH0_OFFSET + (i*2) + 1].reg_d.reg_value = (Data.array[KWH_0_OFFSET + (i*2) + 1] & 0x0000FFFF);
...
void System_Status_Update(void)
{
	...
	{
	  /*op_breaker_trip_status++;
	  if (op_breaker_trip_status >= 5)
	  {*/
	   if(!event_flag[4])
	   {
		dummy1 = 23;
	...
	{
	  /*ip_breaker_trip_status++;
	  if (ip_breaker_trip_status >= 5)
	  {*/
	   if(!event_flag[1])
	   {
		dummy1 = 8;
...
#endif
	//printf("\n Ambient Temp : %d",Data.word.System_Parameter.Ambient_Temp);  //12345
	//printf("Ambient Temp Limit : %x\n",Data_In.word.wSysThd.word.wAmbTempHighLimit);
...
static void icos_test_adc(int addr, int channel)
{
  ...
  //chn_no = 1;
  //chn_addr = 0x4A;
  int fan_err_status = 0, config, fan_status = 0, fan_alarm = 0;
  int os_bit1;
  ...
  int fan_rec_count[8] = {0,0,0,0,0,0,0,0};
	...
	while(1) 
	{
		...
		for(i = 0;i < 8;i++)
		{
			adc_flag = 1;
			fan_alarm = 0x00000001 << i;
			fan_status = get_adc_avg(adc_flag, chn_address[i], chn_number[i], &z_Offset[i], &fail_count[i], &struck_count[i], &fan_rec_count[i]);
			...
			else if (fan_rec_count[i] > (fan_cntr_Parameters[7] - 1))
			{
				fan_err_status &= (~fan_alarm);
			}
		}
...
{
	int i, count, s_count, r_count;
	for(i = 0; i < 8;i++)
	{
		adc_flag = 0;
		get_adc_avg(adc_flag, chn_address[i],chn_number[i],&z_Offset[i], &count, &s_count, &r_count);
	}

}

int get_adc_avg(int adc_flag, int chn_addr, int chn_no, int *avg_value, int *count, int *jam_count, int *fan_rec_count)
{
	...
	int off_count,j_count,rec_count;
    ...
    for(i =0;i < fan_cntr_Parameters[6];i++)
    {
		os_bit = 0;
        ...
        if(os_bit)
        {
			...
			else if(temp < fan_cntr_Parameters[3])   
			{
				off_count++;
			}
			else if(temp < fan_cntr_Parameters[5])//default limit set : 8900
			{
				rec_count++;
			}
        }
	...
	*count = off_count; 
	*jam_count = j_count; 
	*fan_rec_count = rec_count;   
}


/*

int get_adc_avg(chn_addr, chn_no)
{
  nos_uint16 config_data = 0; 
  nos_uint16 conver_data[] = 0;
  ...
  for(i =0;i<10;i++)
  {
    nos_i2c_write_word(1, 0x49, 0x01, 0x93c5);
    system("sleep 0.008s");
    nos_i2c_read_word(1, 0x49,0x01, &config_data);
    os_bit = (config_data >> 7) & 0x0001;
    printf("OS bit : %d\n",os_bit);
    if(os_bit)
    {
      printf("Read start : OS bit is %d\n",os_bit);  
      nos_i2c_read_word(1, 0x49,0x00, &conver_data);
      m_byte = (conver_data & 0xFF) << 8;
      l_byte = (conver_data & 0xFF00) >> 8; 
      f_data = m_byte | l_byte;
      printf("Data : %d\n", f_data);
      printf("Data in hex: %X\n", f_data);
    }
    else
    {
      printf("Read failed: OS bit is %d\n",os_bit);
    }
